...
| 类型 | 说明 | 来源 |
|---|---|---|
| 📅 历史上的今天 | 按"月-日"标注的网球史事件，当天自动置顶「🎂 就在今天！」 | 手工精选 + **真实赛果生成器**（每天保底 3 张、全年 365 天覆盖） |
...
- 喜欢/收藏/作答记录存入 localStorage；卡片顺序按日期种子洗牌（一天内稳定，跨天换序）
...
见 [DEPLOY.md](./DEPLOY.md)。推荐 **GitHub Pages**：上传仓库 → 开启 Pages，
配合 `.github/workflows/` 里的定时任务，**新闻每 3 小时、历史卡片每周自动更新**，无需手动维护。